...
export default function EtsaReviewPage(){
  const params=useParams<{assessmentId:string}>(); const assessmentId=params.assessmentId;
  const [data,setData]=useState<any>(null); const [scores,setScores]=useState<Record<number,number>>({}); const [notes,setNotes]=useState<Record<number,string>>({}); const [error,setError]=useState(""); const [saving,setSaving]=useState<number|null>(null);
  ...
  return <main style={{minHeight:"100vh",background:"#07090c",color:"#f5f7fa",padding:"40px 20px 80px"}}><div style={{width:"min(980px,100%)",margin:"0 auto"}}>
    ...
    <div style={{display:"grid",gap:22,marginTop:30}}>{[66,67,68,69,70].map(id=>{const q=ETSA_QUESTIONS[id-1];const response=responses.get(id);return <section key={id} style={{padding:24,border:"1px solid rgba(255,255,255,.12)",borderRadius:18,background:"rgba(255,255,255,.025)"}}>
      ...
      <button onClick={()=>save(id)} disabled={scores[id]===undefined||saving===id} style={{marginTop:14,padding:"11px 16px",borderRadius:999,border:0,fontWeight:800,cursor:"pointer"}}>{saving===id?"SAVING…":"SAVE SCORE"}</button>
    </section>})}</div>
  </div></main>;
}
