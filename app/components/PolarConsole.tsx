  ...
  return (
    <div ref={consoleRef} className={styles.console} onMouseEnter={() => setTracking(true)} onMouseMove={trackPointer} onMouseLeave={resetTracking}>
      ...
      <div className={`${styles.stage} ${summoned ? styles.summoned : ""}`}>
        ...
        {summoned ? (
          <video
            ...
            src="/media/polar/01_POLAR_Greeting.mp4"
            autoPlay
            muted={!voiceEnabled}
            playsInline
            preload="metadata"
            onEnded={() => setVoiceEnabled(false)}
            aria-label="P.O.L.A.R. greeting transmission"
          />
        ) : (
      ...
      <div className={styles.transmission} aria-live="polite">
        ...
        {summoned ? "Approved P.O.L.A.R. greeting transmission loaded. Voice remains user-controlled." : diagnostic ? "Diagnostic complete. The vision is not too complicated. It is under-architected." : transmissions[messageIndex]}
      </div>
