...
const copyComposition = (source, name) => {
  const directory = path.join(generatedRoot, name);
  fs.mkdirSync(directory, { recursive: true });
  fs.copyFileSync(path.resolve(source), path.join(directory, 'index.html'));
};

copyComposition('compositions/greeting.html', 'greeting');
copyComposition('compositions/idle-inquiry.html', 'idle-inquiry');
